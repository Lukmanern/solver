...
func findMissingNumber(arr []int) int {
	...
	// Calculate the sum of all the numbers from 1 to the length of the array + 1
	// This is the expected sum if there are no missing numbers
	expectedSum := (len(arr) + 1) * (len(arr) + 2) / 2
	
	// Return the difference between the expected sum and the actual sum as the missing number
...
func findMissingNumbers(arr []int) []int {
	...
	// Find the missing numbers by iterating through all the numbers from 1 to the length of the array + 1
	// and checking if they are in the counts map
